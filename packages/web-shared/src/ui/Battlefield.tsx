...
export function Battlefield({ className, engine, backgroundEngine, view, suspended = false, onOpenArchive, utilityPanel, workshop, children }: {
  ...
  return (
    <section className={[styles.root, className].filter(Boolean).join(' ')} data-phase={snapshot.status} data-control={engine.externallyControlled ? 'external' : 'local'} aria-label={t('battlefield.aria')}>
