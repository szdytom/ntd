    ...
    statusOrbs({ id: burningEffectId, size: 3.2, hotColor: brightColor, bloom: 0.72 }),
    {
        ...
        render: (frame, painter) => {
            ...
            for (let index = 0; index < 10; index += 1) {
                ...
                painter.circle(frame.x + Math.cos(angle) * travel, frame.y + Math.sin(angle) * travel, (1.2 + frame.random(index + 40, 2.2, 4.2) * frame.fout) * speed, particleColor, frame.fout);
                if (index % 3 === 0) {
                    painter.lineAngle(frame.x + Math.cos(angle) * travel, frame.y + Math.sin(angle) * travel, angle, 3 + 8 * frame.fout, 1.7 * frame.fout + 0.35, index % 2 === 0 ? brightColor : darkColor, frame.fout * 0.9);
                }
            }
        },
    },
    {
        ...
        render: (frame, painter) => {
            for (let index = 0; index < 7; index += 1) {
                ...
                painter.circle(frame.x + Math.cos(angle) * travel, frame.y + Math.sin(angle) * travel - lift, 1.5 + frame.slope * frame.random(index + 60, 3, 5.8), hot > 0 ? index % 3 === 0 ? color : darkColor : smokeColor, frame.fout * (0.58 + hot * 0.35));
            }
        },
    },
];
export const emberCoatingModule = createDamageStatusModifier({
    ...
    renderProjectile: ({ ctx, projectile }) => {
        ...
        for (const offset of [0, Math.PI]) {
            ctx.beginPath();
            ctx.arc(projectile.position.x + Math.cos(phase + offset) * (projectile.radius + 4), projectile.position.y + Math.sin(phase + offset) * (projectile.radius + 4), 1.8, 0, Math.PI * 2);
            ctx.fill();
        }
        ctx.restore();
    }
