    ...
    statusOrbs({ id: burningEffectId, lifetime: 0.52, size: 4.1, hotColor: brightColor, bloom: 0.88 }),
    {
        ...
        render: (frame, painter) => {
            ...
            for (let index = 0; index < 8; index += 1) {
                ...
                painter.triangle(frame.x + Math.cos(angle) * (10 + frame.easeOut(3) * 19), frame.y + Math.sin(angle) * (10 + frame.easeOut(3) * 19), 5 * frame.fout + 0.5, rayLength * frame.fout, angle, index % 3 === 0 ? brightColor : index % 2 === 0 ? color : darkColor, frame.fout * 0.92);
            }
    ...
    },
    {
        ...
        render: (frame, painter) => {
            for (let index = 0; index < 16; index += 1) {
                ...
                if (index % 3 === 0) {
                    painter.circle(x, y, 1 + frame.slope * frame.random(index + 70, 3.5, 6), frame.fin < 0.52 ? darkColor : smokeColor, frame.fout * 0.74);
                }
                else {
                    const particleColor = frame.fin < 0.16 ? brightColor : frame.fin < 0.6 ? color : darkColor;
                    painter.lineAngle(x, y, angle, frame.random(index + 90, 7, 16) * frame.fout + 2, 2.6 * frame.fout + 0.4, particleColor, frame.fout);
                }
            }
        },
    },
