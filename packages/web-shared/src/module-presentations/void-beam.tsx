...
const wanderingLightOffset = (id: number, time: number, radius: number): {
    x: number;
    y: number;
} => {
    const phase = id * 0.61803398875;
    return {
        x: (Math.sin(time * 0.83 + phase) +
            Math.sin(time * 0.37 + phase * 2.1) * 0.45) * radius * 0.31,
        y: (Math.cos(time * 0.69 + phase * 1.7) +
            Math.sin(time * 0.29 - phase * 0.8) * 0.4) * radius * 0.31,
    };
};
...
const drawOrbParticles = (ctx: CanvasRenderingContext2D, x: number, y: number, ringRadius: number, time: number, id: number, emissive: boolean): void => {
    ctx.save();
    if (emissive)
...
const drawVoidRing = (ctx: CanvasRenderingContext2D, x: number, y: number, radius: number, time: number, id: number): void => {
...
export const voidBeamModule: ModulePresentation = {
    ...
    icon: VoidBeamIcon,
    meta: {
        color, displayColor: '#61a36c', tint: '#e9ffed'
    },
    effects,
    renderProjectile: ({ ctx, projectile }) => {
        drawVoidRing(ctx, projectile.position.x, projectile.position.y, projectile.radius, projectile.age, projectile.id);
    },
    renderProjectileBloom: ({ ctx, projectile }) => {
        ...
        ctx.save();
        ctx.globalCompositeOperation = 'lighter';
        drawOrbParticles(ctx, projectile.position.x, projectile.position.y, ringRadius, projectile.age, projectile.id, true);
