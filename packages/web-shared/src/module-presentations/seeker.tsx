...
export const seekerModule: ModulePresentation = {
    ...
    icon: SeekerIcon,
    meta: {
        color, displayColor: '#168aad', tint: '#e2f4f8'
    },
    effects,
    renderProjectile: ({ ctx, projectile }) => {
        ...
        for (const offset of SEEKER_ORBIT_OFFSETS) {
            ctx.beginPath();
            ctx.arc(projectile.position.x + Math.cos(phase + offset) * (projectile.radius + 5), projectile.position.y + Math.sin(phase + offset) * (projectile.radius + 5), 1.7, 0, Math.PI * 2);
            ctx.fill();
        }
        ctx.restore();
    }
