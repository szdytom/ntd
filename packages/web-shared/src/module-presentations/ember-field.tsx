    ...
    }),
    {
        ...
        render: (frame, painter) => {
            ...
            for (let index = 0; index < 10; index += 1) {
                ...
                painter.circle(x, y, 0.8 + frame.slope * frame.random(index + 60, 2.5, 5), frame.fin < 0.5 ? index % 3 === 0 ? '#ffffff' : brightColor : statusColor, frame.fout * 0.75);
            }
        },
    },
];
export const emberFieldModule: ModulePresentation = {
    ...
    icon: EmberFieldIcon,
    meta: {
        color, displayColor: '#ed7118', tint: '#fff1e8'
    },
    effects,
    renderProjectile: ({ ctx, projectile }) => {
        ...
        for (let index = 0; index < 4; index += 1) {
            ...
            ctx.beginPath();
            ctx.arc(x + Math.cos(angle) * orbit, y + Math.sin(angle) * orbit - smokePhase * 22, 3 + smokePhase * 7, 0, Math.PI * 2);
            ctx.fill();
        }
